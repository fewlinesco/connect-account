...
describe("handleAuthErrors", () => {
  ...
  describe("DynamoDB exceptions", () => {
    const mockedException = new Error();
    mockedException.message =
      "Cannot do operations on a non-existent table` exception is thrown";

    it("should redirect to the login flow if a `Cannot do operations on a non-existent table` exception is thrown", async (done) => {
      expect.assertions(2);
