...
We are using `Husky` to trigger the linting, testing and building of the app before pushing the code to GitHub to prevent unnecessary `Heroku` build.
...
Regarding our main flows for end to end tests (i.e `e2e-test-heroku.yml` & `lint-and-test.yml`), here is the gist of it. 
...
To do so, we first create manually a https://docs.github.com/en/rest/reference/checks#get-a-check-run[check-run] (`init-e2e-test-check` job inside `lint-and-test.yml`). This custom check-run is just initialized at this point, and its `status` is pending.
Once we receive the status of the deployment from our third party actor, we run our end to end test suite (`e2e-tests-heroku` job inside `e2e-test-heroku.yml`) if the review app deployment is a success, or we skip it in the case of a failure during the process.
The last job (`e2e-tests-check-run-status` job inside `e2e-test-heroku.yml`) is used to manage the status of the custom check-run we created previously. 
If the deployment failed, we set its status to failure too. When the end to end test suite can be runned, we wait for it to complete before setting the custom check-run status according to the tests results. 
