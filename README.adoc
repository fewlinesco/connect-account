...
To help with repetitive tasks during the development of a feature, or to help debug, we have added a set Dev button to trigger various action on press. You can find them inside `src/components/dev-buttons`.
To enable them, you wil have to render `<DevButtons/>` inside `_app.tsx`, like this:
