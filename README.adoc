...
To trigger script in the CI context, we chose GitHub Actions.

=== Lighthouse reports

- Docker image: https://github.com/femtopixel/docker-google-lighthouse-puppeteer
- Package: https://github.com/femtopixel/google-lighthouse-puppeteer
...
When SVG are monochromatics (i.e. _icons_) and to improve components reusability, we replace the `fill` attribute value of tags composing SVG, such as `path`, with the CSS variable `currentColor` instead of hardcoded color codes. 
