...
- *next.config.js*: Extended webpack compiler used by NextJS.
...
> Note that, contrary to `errors`, `warnings` do not break testing or building the app.
...
The rule we we follow is that, if a declared type is only used in one file, we locate it in said file. Otherwise, we move it in its own file, under `@types/`.
...
The `globalStyle` object, found in `/src/design-system/globals/globalStyle.tsx`, is used to remove undesired style and behavior found in HTML, and give use full controle on
The `globalStyle` is scoped in the `_app` page component.

=== Theme

You should prioritize theme values instead of arbitrary css values.

The theme structure is defined inside the `src/@types/styled-component.d.ts` definition type file.

The theme values are set inside `src/design-system/theme/lightTheme.ts` file. Each field are se individually, then assigned to the `lightTheme` variable.
...
Sometimes, you want to pass custom props to your styled component. To do so, do as follow:
